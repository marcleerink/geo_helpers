...
def test_get_geojson():
    ...
    assert geojson["features"][0]["geometry"]["coordinates"][0][0] == [
        11.540825,
        50.12874500000001,
    ]
    assert geojson["features"][0]["geometry"]["coordinates"][0][1] == [
        11.56041,
        50.12874500000001,
    ]
    assert geojson["features"][0]["geometry"]["coordinates"][0][2] == [
        11.56041,
        50.13516599999997,
    ]
    assert geojson["features"][0]["geometry"]["coordinates"][0][3] == [
        11.540825,
        50.13516599999997,
    ]
    assert geojson["features"][0]["geometry"]["coordinates"][0][4] == [
        11.540825,
        50.12874500000001,
    ]
...
def test_validate_geojson_wrong_polygon_coordinates(schema):
    geojson = {
        ...
        "features": [
            {
                "type": "Feature",
                "geometry": {"type": "Polygon", "coordinates": [[[1, 2], [3, 4, 5]]]},
            }
        ],
    }
    with pytest.raises(ValidationError):
...
def test_validate_geojson_wrong_multipoint_coordinates(schema):
    geojson = {
        ...
        "features": [
            {
                "type": "Feature",
                "geometry": {"type": "MultiPoint", "coordinates": [[1, 2, 3]]},
            }
        ],
    }
    with pytest.raises(ValidationError):
